...
export const draftStyleToSharedTextProp = (
  formats: OrderedSet<string>,
  styleName?: string,
): {} => {
  const prop = {};
...
export const selectionToBlockRange = (
...
export const blockRangeToSelection = (
...
export const textRangeToBlockTextRangeFromBlocks = (
  absPos: TextRange,
  blocks: ContentBlock[],
): BlockTextRange => {
  const contentPos = {
  ...
  for (const block of blocks) {
    prevEnd++;
    if (
      contentPos.startKey === undefined &&
      absPos.start <= prevEnd + block.getLength()
    ) {
 ...
 */
export const selectionToTextRange = (
  selection: SelectionState,
  content: ContentState,
): TextRange => {
...
export const sharedStringToBlockArary = (
    ...
    (segment: ISegment) => {
      if (segment.type === 'Marker') {
        ...
        characterList = characterList.concat(
          new Array(textSegment.cachedLength).fill(meta),
        );
...
export const getMarkersInBlockRange = (
  sharedString: SharedString,
  startKey: string,
  endKey: string,
): Marker[] => {
  ...
  for (const delta of opRanges) {
    if (delta.operation === MergeTreeDeltaType.INSERT) {
      ...
      if (delta.position < updatedRange.start) {
        ...
        updatedRange.end -= Math.min(
          delta.segment.cachedLength,
          updatedRange.end - delta.position,
        );
      }
    }
  }
